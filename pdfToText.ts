...
*   folderId (string, default blank)     Folder id to store file(s) in.
*   folder (DriveFolder, default blank)  Folder to store file(s) in.
...
export function pdfToText(pdfFile, options?: Options) {
  ...
  if (options.path) {
    ...
    Logger.log("folderId")
    parents.push(DriveApp.getFolderById(options.folderId));
  } else if (options.folder) {
    Logger.log("folder")
    parents = options.folder.getParents();
  }
...
function getDriveFolderFromPath(path: string) {
